...
当前检索到的 `ex-parent` 事实如下：

- 多个模块的 Maven 编译目标为 Java 8。
- `exchange-app-api/build/Dockerfile` 使用 `openjdk:8u342-jdk`，入口命令为 `java -jar`，没有显式 GC 参数。
...
2. “演进建议”：以 JDK 17/21 等现代 LTS 为前提，提供 G1 或 ZGC 的实验配置，不把建议配置描述为当前线上配置。
...
- JDK 版本边界：当前 Java 8 镜像不能直接使用现代 ZGC；迁移必须先升级 JDK 和运行时镜像。
...
- 以 `ex-parent` 当前 Java 8 状态为起点的分阶段路线：先观测、再升级 JDK、再压测、最后选择收集器。
