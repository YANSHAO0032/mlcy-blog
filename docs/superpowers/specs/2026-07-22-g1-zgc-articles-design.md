...
当前检索到并由项目实际构建环境确认的 `ex-parent` 事实如下：

- 真实构建使用 JDK 21，但多个模块的 Maven 编译目标仍为 Java 8，表示构建 JDK 与源码兼容级别是两个维度。
- `exchange-app-api/build/Dockerfile` 当前文件内容使用 `openjdk:8u342-jdk`，入口命令为 `java -jar`，没有显式 GC 参数；文章不能仅凭构建 JDK 21 推断线上运行时已经是 JDK 21。
...
- 仓库检索范围内没有发现 `-XX:+UseG1GC`、`-XX:+UseZGC`、`-Xlog:gc` 等显式 JVM 启动配置。

正文必须区分两类信息：

1. “当前工程”：只陈述可以从源码、POM 或 Dockerfile 直接确认的事实。
2. “演进建议”：以 JDK 21 运行时为前提，提供 G1 或 ZGC 的实验配置；如果部署镜像仍是 Java 8，则先说明必须同步升级运行时镜像，不能直接启用 ZGC。
...
- JDK 版本边界：真实构建使用 JDK 21，但当前检索到的 Java 8 运行时镜像不能直接使用现代 ZGC；只有确认部署运行时也升级到 JDK 21 后，才把 ZGC 作为可执行方案。
...
- 迁移风险：JDK 升级兼容性、监控字段变化、容器内存预算、吞吐回退和压测方法。
...
- 以 `ex-parent` “构建 JDK 21、源码兼容 Java 8、运行镜像需核实”的状态为起点，设计先确认运行时、再观测、再压测、最后选择收集器的路线。
